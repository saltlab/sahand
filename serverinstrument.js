...
exports.instrument = function() {
    var appFiles = fs.readdirSync('sample-app-code');
    ...
    appFiles.forEach(function (fileName) {
        ...
        if ('.html' == fileExt || '.htm' == fileExt) {
            ...
            console.log('js');
            var filePath = 'sample-app-code/' + fileName;
            var script = fs.readFileSync(filePath, 'utf8');
            var instrumentedAst = instrumentAst(script, filePath);
...
function instrumentAst(script, path) {
